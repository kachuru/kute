...
class DealOrNoDealCommand extends Command
{
    ...
    public function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        for ($gameNumber = 1; $gameNumber <= $games; $gameNumber++) {
            ...
            for ($turn = 1; $turn <= 20; $turn++) {
                ...
                if ($this->shouldBankerOffer($turn)) {
                    $output->writeln(sprintf('Banker offer: <comment>£%d</comment>', $game->bankerOffer($playerBox)));
                }
            }

            $finalBox = $game->draw();
            $output->writeln(sprintf('  Final Box: <comment>%d</comment>. It contained <error>%s</error>', $finalBox->getNumber(), $this->getValueToShow($finalBox->getValue())));
            $output->writeln(sprintf('  Player\'s box held <error>%s</error>', $this->getValueToShow($playerBox->getValue())));
        ...
        if ($games > 1) {
            $output->writeln(sprintf('Played <info>%d</info> games. Won %d. Lost %d. (%0.2f%%)', $games, $won, $lost, (100/$games)*$won));
        }
